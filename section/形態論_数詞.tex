\langname の基数詞は以下の通りである。
\begin{itemize}
    \item 1: uea
    \item 2: vai
    \item 3: vavi
\end{itemize}

これらの数詞は、名詞、そしてそれに続く類別詞とともに使用される。
\begin{exe}
    \ex \gll uea-pi \\
    one-CLF.human \\
    \glt 一人
\end{exe}
% TODO : 例文の検証：例文(3)は「数詞＋類別詞」であり、数詞が名詞を修飾する**名詞句**の構造を示す例文としては不適切です。名詞 (*uea* '一人') が類別詞を伴って名詞句を形成する一般的な例文（例：*ueo uea-pi* '一人の子供'）を提示してください。

\begin{exe}
    \ex \gll tui vai-mu \\
    dog two-CLF.animal \\
    \glt 二匹の犬
\end{exe}
% TODO : 形態素境界の確認：例文(4)では *vai-mu* とハイフンで繋がれていますが、これは数詞 *vai* と類別詞 *mu* が**接辞**または**接語**の関係にあることを示唆します。数詞と類別詞が独立した語ではなく、複合体として機能することを明記し、この複合体の内部構造（例：[数詞-類別詞]が複合語として機能するのか）を分析してください。

4以上の数詞は\langname の本来語にはない。
近隣言語から借用された数詞が使われることがある。

% TODO : 4以上の数詞の使用例：借用された数詞（例：4, 5, 10など）がどのように\langname の音韻論に適合しているか、および、それらが類別詞を伴って使用される際の例文を提示してください。

数詞が名詞句に含まれる場合、類別詞に複数接尾辞（-ia）は付加されない。
\begin{exe}
    \ex \gll kuaia mu-ia \\
    pig CLF.animal-PL \\
    \glt 豚たち
\end{exe}
\begin{exe}
    \ex \gll kuaia vavi-mu \\
    pig three-CLF.animal \\
    \glt 三頭の豚
\end{exe}
% TODO : グロスの表記の修正：例文(6)のグロス *vavi-mu* は「three-CLF.animal」となっていますが、数詞と類別詞が複合体であるならば、一語として *vavi.mu* のようにドットで区切るか、あるいは独立した語として *vavi mu* とグロスし、その統語的地位を明確にしてください。
% TODO : 複数形の抑制の検証：数詞がある場合でも、類別詞に複数接尾辞 *-ia* が付加されないことを示す**非文法的な例文**（例：*\gll kuaia vavi-mu-ia*）を提示し、この制約の厳密性を裏付けてください。